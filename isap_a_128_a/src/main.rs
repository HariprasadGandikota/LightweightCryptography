...
fn main() -> io::Result<()>{
    ...
    for line in reader.lines() {
        ...
        if line.trim().is_empty() {
            if !key.is_empty() && !nonce.is_empty() && !ct.is_empty() {
                ...
                let result = verify(key.clone(), nonce.clone(), &pt.clone(), &ad.clone(), &ct.clone());
        ...
        if line.starts_with("Count = ") {
            _count = line["Count = ".len()..].parse().unwrap();
        } else if line.starts_with("Key = ") {
            hex::decode_to_slice(&line["Key = ".len()..], &mut key).expect("Invalid hex string");
    ...
    println!("average elapsed time for 1089 test cases is: {:?}", avg_elapsed);
    println!("average throughput for 1089 test cases is: {:.2?}mbps", avg_throughput);
    Ok(())
}