...
impl TinyjambuAead {
    pub fn new(key: &[u8;24], nonce: &[u8;12]) -> Self {
        Self { key: *key,
            nonce: *nonce,
            state: [0u32;4],
        }
    ...
    fn state_update(&mut self, rounds: usize) {
        let u32_key: [u32;6] = u8_to_u32_array(&self.key).try_into().unwrap();
        for i in 0..(rounds>>5) {
...
pub fn verify192(key: &[u8;24], nonce: &[u8;12], pt: &Vec<u8>, ad: &Vec<u8>) -> bool {
...
mod tests {
    ...
    #[test]
    fn it_works() {
        let key: [u8;24] = (hex::decode("000102030405060708090A0B0C0D0E0F1011121314151617").expect("Decoding failed")).as_slice().try_into().expect("incorrect length");
